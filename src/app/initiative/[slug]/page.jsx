...
async function getData(slug) {
  try {
    const res = await fetch(
      "http://127.0.0.1:1337/api/initiatives/" + slug + "?populate=*",
      { next: { revalidate: 1 } }
    );
...
export default async function page({ params }) {
  ...
  console.log(response);
  // console.log(response.data.attributes.description);
    ...
    <>
      <div className="bg-white px-6 py-12 lg:px-8">
        <div className="mx-auto max-w-3xl text-base leading-7 text-gray-700">
          ...
          <Markdown>{response.data.attributes.description}</Markdown>
          ...
          <figure className="mt-16">
            <img
              ...
              src={`http://127.0.0.1:1337${response.data.attributes.thumbnail.data.attributes.url}`}
              alt=""
            />
